...
bool shield, tilt, dolphin = 0, off = 0, back_off = 0;
...
int dcx, dcy, dx, dy, dl, dr;

void convertinputs(){
  //new mods by /u/digigon1
  native_input();  //enables native input (NEEDS TESTING)

  if(!back_off){
  no_light_shield();  //disables light shield
...
void no_light_shield(){
  gcc.left = (gcc.left>250)?255:0;
  //second attempt
  //gcc.left = (gcc.l)?255:0;
}

void native_input(){
  gcc.cxAxis = max(gcc.cxAxis - dcx, 0);
  gcc.cyAxis = max(gcc.cyAxis - dcy, 0);
  gcc.xAxis = max(gcc.xAxis - dx, 0);
  gcc.yAxis = max(gcc.yAxis - dy, 0);
  gcc.left = max(gcc.left - dl, 0);
  gcc.right = max(gcc.right - dr, 0);
}
...
void nocode(){
  ...
  if(gcc.ddown){
    if(m == 0) m = millis();
    back_off = back_off||(millis()-m>2000);
  }else m = 0;
}
